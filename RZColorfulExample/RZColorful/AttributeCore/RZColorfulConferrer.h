 ...
 */
- (RZColorfulAttribute *(^)(NSString * _Nullable))text;
/** 添加html格式的内容(网页源码)，会通过将源码转换成普通文本内容 */
- (RZColorfulAttribute *(^)(NSString * _Nullable))htmlText;
/** 添加图片，为使图片与前后排文字对齐，可在后设置size */
...
@interface RZColorfulConferrer (Replace)
/** 已废弃 */
- (RZImageAttachment *(^)(UIImage * _Nullable))appendImage RZWARNING("替换成 image");
/** 已废弃 */
- (RZImageAttachment *(^)(NSString * _Nullable))appendImageByUrl RZWARNING("替换成 imageByUrl");
@end
